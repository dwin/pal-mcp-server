...
class TestAliasTargetRestrictions:
    ...
    def test_critical_regression_validation_sees_alias_targets(self):
        ...
        - A restriction on a canonical target would not be recognized as valid

        After the fix:
        - list_models(respect_restrictions=False, include_aliases=True, lowercase=True, unique=True) returns aliases + targets
        - validate_against_known_models() checks against all names
        - A restriction on a canonical target is recognized as valid
        """
